...
```bash
helm install vector-syslog ./vector-copilot \
  -n <namespace> \
...
| `tls.secretName` | TLS certificate secret name | `vector-tls` |
| `env.ingestTokenSecretName` | Azure token secret name | `azure-dcr-token` |
...
# Send a CEF message
echo 'CEF:0|Security|Firewall|1.0|100|Connection Denied|5| src=10.0.0.1 dst=192.168.1.1' | \
...
```bash
kubectl logs -n <namespace> -l app.kubernetes.io/name=vector-copilot -f
```
...
Check events:
```bash
kubectl describe pod -n <namespace> -l app.kubernetes.io/name=vector-copilot
```
...
Verify certificate:
```bash
kubectl get secret vector-tls -n <namespace> -o yaml
```
...
```bash
helm upgrade vector-syslog ./vector-copilot \
  -n <namespace> \
  -f custom-values.yaml
```
